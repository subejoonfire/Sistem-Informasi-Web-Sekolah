...
class Galeri extends CI_Controller{
	...
	public function update_galeri()
	{
		...
		$this->upload->initialize($config);
		$id_galeri = $this->input->post('kode');
		$judul = strip_tags($this->input->post('xjudul'));
		if (!empty($_FILES['filefoto']['name'])) {
			if ($this->upload->do_upload('filefoto')) {
				$gbr = $this->upload->data();
				...
				$user_id = $p['id_admin'];
				$user_nama = $p['nama_admin'];
				$this->m_galeri->update_galeri($id_galeri, $judul, $user_id, $user_nama, $gambar);
				...
				redirect('admin/galeri');
							...
							$user_id=$p['id_admin'];
							$user_nama=$p['nama_admin'];
							$this->m_galeri->update_galeri_tanpa_img($id_galeri,$judul,$user_id,$user_nama);
							echo $this->session->set_flashdata('msg','info');
							redirect('admin/galeri');
	            } 
	}
